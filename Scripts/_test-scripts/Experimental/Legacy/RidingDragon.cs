...
namespace Server.Mobiles
{
	...
	public class RidingDragon : BaseMount
	{
		...
		[Constructable]
		public RidingDragon( string name, int body, int hue ) : base( name, 59, 586, AIType.AI_Mage, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			rBody = body;
			Hue = hue; 
			BaseSoundID = 362;
		...
		{
			if ( Server.Misc.MyServerSettings.ClientVersion() && CharacterDatabase.GetKeys( from, "DragonRiding" ) )
		...
		public RidingDragon( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnAfterSpawn()
		{
			...
			else if ( reg.IsPartOf( typeof( VillageRegion ) ) )
			{
				string[] sCategory = new string[] { "dungeon", "fire", "land", "snow", "sky", "swamp", "sea", "mountain", "jungle", "forest", "sand" };
				category = sCategory[Utility.RandomMinMax( 0, (sCategory.Length-1) )];
			}
			else
			{
