...
namespace Server.Mobiles
{
	public class Mapmaker : BaseVendor
	{
		...
		[Constructable]
		public Mapmaker() : base( MapmakerStringConstants.TITLE_MAPMAKER )
		{
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is TreasureMap && from.Backpack != null)
                {
                    ...
                    else if (pack.ConsumeTotal(typeof(Gold), toConsume))
                    {
						...
						if ( tmap.Level == 1 ){ m_Mapmaker.SayTo(from, "This map was really quite simple."); }
						else if ( tmap.Level == 2 ){ m_Mapmaker.SayTo(from, "Seemed pretty easy...so here it is."); }
						else if ( tmap.Level == 3 ){ m_Mapmaker.SayTo(from, "This map was a bit of a challenge."); }
						else if ( tmap.Level == 4 ){ m_Mapmaker.SayTo(from, "Whoever drew this map, did not want it found."); }
						else if ( tmap.Level == 5 ){ m_Mapmaker.SayTo(from, "This took more research than normal."); }
						else { m_Mapmaker.SayTo(from, "With the ancient writings and riddles, this map should now lead you there."); }
