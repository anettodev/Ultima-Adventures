...
{
	/// <summary>
	/// Cunning - 2nd Circle Buff Spell
	/// Increases target's Intelligence temporarily
	/// </summary>
	public class CunningSpell : MagerySpell
	{
		...
		public void Target(Mobile target)
		{
			...
			else if (CheckBSequence(target))
			{
				...
				PlayEffects(target);
				BuffInfo.AddBuff(target, new BuffInfo(BuffIcon.Cunning, 1075843, duration, target, percentage.ToString()));
			}
		...
		public class InternalTarget : Target
		{
			...
			public InternalTarget(CunningSpell owner) : base(Core.ML ? TARGET_RANGE_ML : TARGET_RANGE_LEGACY, false, TargetFlags.Beneficial)
			{
				m_Owner = owner;
			}
