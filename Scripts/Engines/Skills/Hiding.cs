...
namespace Server.SkillHandlers
{
	...
	if ( Server.Items.BaseInvisibilityPotion.HasActiveEffect( m ) )
	{
		...
		double bonus = 0.0;
			...
			if ( m.CheckSkill( SkillName.Hiding, 75, 250 ) ){ badCombat = false; } // ADDED A LITTLE EXTRA HIDING ABILITY FOR HIGH SKILL
			...
			else 
			{
				if ( ok )
				{
					...
					{
						if ( pet is BaseCreature && (int)m.GetDistanceToSqrt( pet ) < 14 && m.InLOS( pet )  )
						{
