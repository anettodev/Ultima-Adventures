...
namespace Server.Regions
{
    public class GuardedRegion : BaseRegion
    {
        ...
        public virtual Type DefaultGuardType
        {
            get
            {
                if (this.Map == Map.Ilshenar || this.Map == Map.Malas)
                    return typeof(YewGuard);
                else
                    return typeof(BritainGuard);
            }
        }
        ...
        {
            BaseRanger useGuard = null;

            foreach (Mobile m in focus.GetMobilesInRange(8))
            {
                if (m is BaseRanger)
                {
                    BaseRanger g = (BaseRanger)m;
        ...
        {
            if (m is BaseRanger || !m.Alive || m.AccessLevel > AccessLevel.Player || m.Blessed || IsDisabled())
                return false;
