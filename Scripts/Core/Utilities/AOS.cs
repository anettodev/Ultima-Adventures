		...
		public static int Damage( Mobile m, Mobile from, int damage, bool ignoreArmor, int phys, int fire, int cold, int pois, int nrgy, int chaos, int direct, bool keepAlive, bool archer, bool deathStrike, bool magic, bool nuking, bool potions )
		{
			...
			if ( from is PlayerMobile && ((PlayerMobile)from).Avatar ) // new effect - balance affects players damage
			{
				...
				if (AetherGlobe.BalanceLevel > 51000) //balance is evil
				{
					...
					adjust = (int)(bal * (double)totalDamage);
			...
			if( from != null && !from.Deleted && from.Alive )
			{
				...
				if (reflectPhys > reflectDamageCap) {
	...
	public sealed class AosAttributes : BaseAttributes
	{
		...
		public static int GetValue( Mobile m, AosAttribute attribute )
		{
			...
			if (m is PlayerMobile) {
				...
				if (phylactery != null) {
					AosAttributes attrs = phylactery.Attributes;	
					if( attrs != null ) {
			...
			if( attribute == AosAttribute.Luck && m is PlayerMobile)
			{
				if ( ((PlayerMobile)m).Avatar )
					value = (int)((double)value * 1.15);
				else if ( !((PlayerMobile)m).Avatar )
					value = (int)((double)value * 0.75);
			}
		...
		{
			int strBonus = Server.Misc.AdventuresFunctions.DiminishingReturns(BonusStr, Misc.MyServerSettings.StatBonusCap());
			int dexBonus = Server.Misc.AdventuresFunctions.DiminishingReturns(BonusDex, Misc.MyServerSettings.StatBonusCap());
			int intBonus = Server.Misc.AdventuresFunctions.DiminishingReturns(BonusInt, Misc.MyServerSettings.StatBonusCap());
