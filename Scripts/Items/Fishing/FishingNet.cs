...
namespace Server.Items
{
	public class FishingNet : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendMessage(55, "Voc� n�o tem habilidade suficiente em pescar para usar esta rede.");
			}
			else if ( Worlds.IsOnBoat( from ) == false )
			{
				from.SendMessage(55, "Voc� precisar� estar no seu barco para usar esta rede.");
			}
			else if ( Worlds.BoatToCloseToTown( from ) == true )
			{
				from.SendMessage(55, "Voc� precisar� ir para �guas mais profundas para usar esta rede.");
			}
			else if ( IsChildOf( from.Backpack ) )
			{
                from.SendMessage(55, "Onde voc� deseja usar a rede de pesca?");
		...
		public void OnTarget( Mobile from, object obj )
		{
			...
			{
                from.SendMessage(55, "Voc� precisa estar mais perto da �gua para pescar!");
			...
			else if ( hasWater )
			{
                ...
                from.SendMessage(55, "Voc� mergulha a rede no mar");
                //from.SendLocalizedMessage( 1010487 ); // You plunge the net into the sea...
			}
			else
			{
				from.SendMessage(55, "Voc� s� pode usar esta rede em �guas profundas!");
