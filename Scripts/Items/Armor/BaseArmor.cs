...
namespace Server.Items
{
	public abstract class BaseArmor : Item, IScissorable, ICraftable, IWearableDurability
	{
		...
		public virtual double ArmorRating
		{
			...
			// Apply protection level bonus
			if ( m_Protection != ArmorProtectionLevel.Regular )
				ar += ArmorConstants.PROTECTION_BASE_BONUS + (ArmorConstants.PROTECTION_LEVEL_MULTIPLIER * (int)m_Protection);

			// Apply craft resource bonus using dictionary lookup (replaces 50+ case switch statement)
			ar += ArmorConstants.GetResourceArmorBonus(m_Resource);

			// Apply quality modifier
			ar += ArmorConstants.QUALITY_POOR_PENALTY + (ArmorConstants.QUALITY_LEVEL_BONUS * (int)m_Quality);
		...
		public int ComputeStatReq( StatType type )
		{
			...
			return AOS.Scale( v, ArmorConstants.STAT_REQUIREMENT_BASE - GetLowerStatReq() );
		}
		...
		public int GetProtOffset()
		{
			switch ( m_Protection )
			{
				...
				case ArmorProtectionLevel.Hardening: return ArmorConstants.PROTECTION_HARDENING_OFFSET;
				case ArmorProtectionLevel.Fortification: return 3;
				case ArmorProtectionLevel.Invulnerability: return ArmorConstants.PROTECTION_INVULNERABILITY_OFFSET;
			}
		...
		{
			int scale = ArmorConstants.DURABILITY_SCALE_BASE + GetDurabilityBonus();

			m_HitPoints = ((m_HitPoints * ArmorConstants.DURABILITY_SCALE_BASE) + (scale - 1)) / scale;
			m_MaxHitPoints = ((m_MaxHitPoints * ArmorConstants.DURABILITY_SCALE_BASE) + (scale - 1)) / scale;
			InvalidateProperties();
		}
		...
		public void ScaleDurability()
		{
			...
			m_HitPoints = ((m_HitPoints * scale) + ArmorConstants.DURABILITY_ROUNDING_OFFSET) / ArmorConstants.DURABILITY_SCALE_BASE;
			m_MaxHitPoints = ((m_MaxHitPoints * scale) + ArmorConstants.DURABILITY_ROUNDING_OFFSET) / ArmorConstants.DURABILITY_SCALE_BASE;
			InvalidateProperties();
		}
		...
		public int GetDurabilityBonus()
		{
			...
			if ( m_Quality == ArmorQuality.Exceptional )
				bonus += ArmorConstants.EXCEPTIONAL_DURABILITY_BONUS;
		...
		public bool Scissor( Mobile from, Scissors scissors )
		{
			...
			if ( item != null && item.Resources.Count == 1 && item.Resources.GetAt( 0 ).Amount >= ArmorConstants.SCISSOR_MIN_RESOURCES )
			{
				try
				{
					Item res = (Item)Activator.CreateInstance( CraftResources.GetInfo( m_Resource ).ResourceTypes[0] );

					ScissorHelper( from, res, m_PlayerConstructed ? (item.Resources.GetAt( 0 ).Amount / ArmorConstants.SCISSOR_RESOURCE_DIVISOR) : 1 );
					return true;
				}
				catch
				{
		...
		private static double[] m_ArmorScalars = ArmorConstants.GetArmorScalars();
				...
				if ( item is BaseArmor )
				{
					...
					if( armor.RequiredRace != null && m.Race != armor.RequiredRace )
					{
						if( armor.RequiredRace == Race.Elf )
							...
							m.SendMessage( ArmorStringConstants.MSG_RACE_ONLY_FORMAT, armor.RequiredRace.PluralName );
					...
					else if ( !armor.AllowMaleWearer && !m.Female && m.AccessLevel < AccessLevel.GameMaster )
					{
						if ( armor.AllowFemaleWearer )
							...
							m.SendMessage( ArmorStringConstants.MSG_CANNOT_WEAR );
		...
		public int GetLowerStatReq()
		{
			...
			if ( v > ArmorConstants.WEAR_MAX_VALUE )
				v = ArmorConstants.WEAR_MAX_VALUE;
		...
		{
			int scale = ArmorConstants.DURABILITY_SCALE_BASE;

			if ( m_MaxHitPoints > 0 && m_HitPoints < m_MaxHitPoints )
				scale = ArmorConstants.DAMAGED_ARMOR_MIN_SCALE + ((ArmorConstants.DAMAGED_ARMOR_MAX_SCALE * m_HitPoints) / m_MaxHitPoints);

			return ( armor * scale ) / ArmorConstants.DURABILITY_SCALE_BASE;
		}
			...
			if( from.AccessLevel < AccessLevel.GameMaster )
			{
				if( RequiredRace != null && from.Race != RequiredRace )
				{
					if( RequiredRace == Race.Elf )
						...
						from.SendMessage( ArmorStringConstants.MSG_RACE_ONLY_FORMAT, RequiredRace.PluralName );
				...
				else if( !AllowMaleWearer && !from.Female )
				{
					if( AllowFemaleWearer )
						...
						from.SendMessage( ArmorStringConstants.MSG_CANNOT_WEAR );
				...
				else
				{
					...
					{
						from.SendMessage( ArmorStringConstants.MSG_INSUFFICIENT_INT );
						return false;
					}
				}
			}
		...
		public override bool OnEquip( Mobile from )
		{
			...
			if( base.OnEquip( from ) )
			{
				...
				{
					from.SendMessage( ArmorStringConstants.MSG_COWL_COLOR_HINT );
				}
			}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( from is PlayerMobile && from.FindItemOnLayer( Layer.Gloves ) == this )
			{
				if ( ((PlayerMobile)from).BalanceEffect != 0 )
				{
					...
					{
						if (from.Karma >= 0)
							from.SendMessage( ArmorStringConstants.MSG_CHALLENGE_HONOR );
						else if (from.Karma <0)
							from.SendMessage( ArmorStringConstants.MSG_CHALLENGE_HUMILIATE );
						
						from.Target = new ChampionChallengeTarget(this); // Call our target	
					}
					else 
						from.SendMessage( ArmorStringConstants.MSG_CHALLENGE_COOLDOWN );
				}
				else
				{
					from.SendMessage( ArmorStringConstants.MSG_CHALLENGE_NOT_PLEDGED );
				}
			}
			...
			if ( Server.Misc.MaterialInfo.IsCowlHood( this ) )
			{
				...
				{
					from.SendMessage( ArmorStringConstants.MSG_MUST_WEAR_TO_CHANGE );
				}
				else
				{
					Target t;

					from.SendMessage( ArmorStringConstants.MSG_MATCH_COLOR_PROMPT );
		...
		private class ChallengeTimer : Timer
		{
			...
			public ChallengeTimer( Mobile from ) : base( TimeSpan.FromSeconds( ArmorConstants.CHALLENGE_COOLDOWN_SECONDS ) )
			{
				m_From = from;
			}
				...
				if ( ((PlayerMobile)from).BalanceStatus != ((PlayerMobile)opponent).BalanceStatus)			
				{
					...
					if ( from.BeginAction( this ) )
					{
							...
							if ( (AetherGlobe.EvilChamp == champ || AetherGlobe.GoodChamp == champ) && (AetherGlobe.EvilChamp == opponent || AetherGlobe.GoodChamp == opponent))
							{
								...
								((PlayerMobile)opponent).BalanceEffect = (int)((double)(((PlayerMobile)opponent).BalanceEffect)*ArmorConstants.CHAMPION_DEFEAT_PENALTY);
								((PlayerMobile)from).BalanceEffect += (int)( (double)(old - ((PlayerMobile)opponent).BalanceEffect) * ArmorConstants.CHAMPION_GAIN_RATIO );
							}
							...
							else
							{
								...
								((PlayerMobile)from).BalanceEffect += (int)((double)(((PlayerMobile)opponent).BalanceEffect)*ArmorConstants.BALANCE_TRANSFER_WINNER);
								((PlayerMobile)opponent).BalanceEffect = (int)((double)(((PlayerMobile)opponent).BalanceEffect)*ArmorConstants.BALANCE_TRANSFER_LOSER);
		...
		private class HatTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is Item )
				{
					...
					if ( from.FindItemOnLayer( Layer.Helm ) != m_Hats ) { from.SendMessage( ArmorStringConstants.MSG_MUST_WEAR_TO_CHANGE ); }
					...
					{
						from.SendMessage( ArmorStringConstants.MSG_MATCH_EQUIPPED_ONLY );
					}

					if ( color > 0 )
					{
						m_Hats.Hue = color;
						from.SendMessage( ArmorStringConstants.MSG_COLOR_CHANGED );
					}
					else
					{
						from.SendMessage( ArmorStringConstants.MSG_ITEM_NEEDS_COLOR );
					}
				}
				else
				{
					from.SendMessage( ArmorStringConstants.MSG_MATCH_DISTINCT_COLORS );
				}
			}
		}
		...
		public virtual int OnHit( BaseWeapon weapon, int damageTaken )
		{
			...
			double HalfAr = ArmorRating / ArmorConstants.ARMOR_ABSORPTION_DIVISOR; // Quarter instead of half
			...
			if ( Absorbed < ArmorConstants.MIN_ABSORBED_DAMAGE )
				Absorbed = ArmorConstants.MIN_ABSORBED_DAMAGE;

			int ruin = ArmorConstants.DURABILITY_LOSS_CHANCE; // 25% chance to lower durability

			if ( Server.Misc.MaterialInfo.IsAnyKindOfMetalItem( (Item)this ) )
				ruin = ArmorConstants.METAL_DURABILITY_LOSS_CHANCE; // Metal armor is just more durable and it gives life back to metal armor types

			if ( ruin < Utility.Random( ArmorConstants.DURABILITY_RANDOM_RANGE ) ) // chance to lower durability
			{
				if ( this is ILevelable && !(AdventuresFunctions.IsInMidland((object)this)) )
				{
					LevelItemManager.RepairItems( ((Mobile)Parent) );
				}
				else if ( !AdventuresFunctions.IsInMidland((object)this) && m_AosArmorAttributes.SelfRepair > Utility.Random( ArmorConstants.SELF_REPAIR_CHANCE_DIVISOR ) )
				{
					HitPoints += 1;
				}
				else
				{
					...
					if ( weapon.Type == WeaponType.Bashing )
						...
						wear = Utility.Random( ArmorConstants.RANDOM_WEAR_RANGE );
		...
		public void WearAndTear( int amount )
		{
			...
			repair += ArmorConstants.WEAR_BONUS_MULTIPLIER * ((double)m_HitPoints/ArmorConstants.WEAR_MAX_DURABILITY); // bonus for items with high durability e.g. 255 versus 60.  
			...
			if ( (Utility.RandomDouble()/ ArmorConstants.WEAR_RANDOM_DIVISOR) > repair )
		...
		public override void GetProperties( ObjectPropertyList list )
		{
			...
			if ( (prop = ArtifactRarity) > 0 )
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.FORMAT_ARTIFACT_RARITY, prop ),
					ArmorStringConstants.COLOR_PURPLE ) );

			// ============================================
			// SECTION 2: ARMOR RATING & DEFENSE
			// ============================================

			// Armor rating with color coding based on value
			// Note: AR is handled by base class AddResistanceProperties
			// but we'll add meditation info here

			// Meditation allowance
			// Note: This is calculated and displayed elsewhere in the tooltip system

			// Defense chance (combat defense)
			...
			// ============================================
			// SECTION 6: MAGIC PROPERTIES (PINK/BLUE)
			// ============================================

			// Faster Cast Recovery (Pink - casting property)
			if ( (prop = m_AosAttributes.CastRecovery) != 0 && !md)
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_FASTER_CAST_RECOVERY, prop ),
					ArmorStringConstants.COLOR_PINK ) );

			// Faster Casting (Pink - casting property)
			if ( (prop = m_AosAttributes.CastSpeed) != 0 && !md)
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_FASTER_CASTING, prop ),
					ArmorStringConstants.COLOR_PINK ) );

			// Lower Mana Cost (Blue - mana property)
			if ( (prop = m_AosAttributes.LowerManaCost) != 0 && !md)
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_LOWER_MANA_COST, prop ),
					ArmorStringConstants.COLOR_BLUE ) );

			// Lower Reagent Cost (Pink - casting property)
			if ( (prop = m_AosAttributes.LowerRegCost) != 0 )
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_LOWER_REAGENT_COST, prop ),
					ArmorStringConstants.COLOR_PINK ) );

			// Mage Armor (Pink - magic armor property)
			if ( (prop = m_AosArmorAttributes.MageArmor) != 0 )
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					ArmorStringConstants.LABEL_MAGE_ARMOR,
					ArmorStringConstants.COLOR_PINK ) );

			// Spell Channeling (Pink - casting property)
			if ( (prop = m_AosAttributes.SpellChanneling) != 0 )
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					ArmorStringConstants.LABEL_SPELL_CHANNELING,
					ArmorStringConstants.COLOR_PINK ) );

			// Spell Damage Increase (Pink - spell damage)
			if ( (prop = m_AosAttributes.SpellDamage) != 0 && !md)
				list.Add( 1060483, prop.ToString() ); // spell damage increase ~1_val~%

			// ============================================
			// SECTION 7: STAT BONUSES & REGENERATION
			// ============================================
			...
			// Mana increase (Blue - mana property)
			if ( (prop = m_AosAttributes.BonusMana) != 0 && !md)
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_MANA_INCREASE, prop ),
					ArmorStringConstants.COLOR_BLUE ) );
			...
			// Mana regeneration (Blue - mana property)
			if ( (prop = m_AosAttributes.RegenMana) != 0 && !md)
				list.Add( 1053099, ItemNameHue.UnifiedItemProps.SetColor(
					String.Format( ArmorStringConstants.LABEL_MANA_REGENERATION, prop ),
					ArmorStringConstants.COLOR_BLUE ) );
			...
			// Armor classification indicator (Cyan - informational)
			list.Add(ItemNameHue.UnifiedItemProps.SetColor(ArmorStringConstants.LABEL_CONSIDERED_ARMOR, ArmorStringConstants.COLOR_CYAN));
		}
		...
		public int OnCraft( int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CraftItem craftItem, int resHue )
		{
			...
			if( Quality == ArmorQuality.Exceptional )
			{
				if ( !( Core.ML && this is BaseShield ))		// Guessed Core.ML removed exceptional resist bonuses from crafted shields
					DistributeBonuses( (tool is BaseRunicTool ? ArmorConstants.EXCEPTIONAL_RESIST_BONUS_RUNIC : ArmorConstants.EXCEPTIONAL_RESIST_BONUS_STANDARD) ); // Not sure since when, but right now 15 points are added, not 14.

				if( Core.ML && !(this is BaseShield) )
				{
					int bonus = (int)(from.Skills.ArmsLore.Value / ArmorConstants.ARMSLORE_BONUS_DIVISOR);
