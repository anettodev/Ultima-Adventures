...
namespace Server.Items
{
	...
	public class Moongate : Item
	{
		...
		public virtual void UseGate( Mobile m )
		{
			...
			string world = Worlds.GetMyWorld( m_TargetMap, m_Target, m_Target.X, m_Target.Y );
			...
			else if ( m is PlayerMobile && !CharacterDatabase.GetDiscovered( m, world ))
			{
				m.SendMessage( "You don't know where the gate leads and change your mind." );
			}
			else if ( m_TargetMap != null && m_TargetMap != Map.Internal )
			{

				if (m is PlayerMobile && m.Karma < -500 && ((PlayerMobile)m).Criminal && ( (Server.Misc.Worlds.GetRegionName( m_TargetMap, m_Target ) == "DarkMoor") || (Server.Misc.Worlds.GetRegionName( m_TargetMap, m_Target ) == "the Temple of Praetoria") ) && m_TargetMap == Map.Ilshenar)
					((PlayerMobile)m).Criminal = false;
