...
namespace Server.Items
{
	public class Runebook : Item, ISecurable, ICraftable
	{
		...
		{
			ItemID = Utility.RandomList( 0x0F3D, 0x5687 );
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			...
			else if ( dropped is RecallScroll || dropped is GateTravelScroll || dropped is GraveyardGatewayScroll || dropped is HellsGateScroll || dropped is AstralTravelScroll || dropped is NaturesPassagePotion || dropped is MushroomGatewayPotion )
			{
				if ( m_CurCharges < m_MaxCharges )
				{
					bool jars = false;
					if ( dropped is RecallScroll || dropped is GateTravelScroll || dropped is AstralTravelScroll )
					{
					...
					if ( amount > (m_MaxCharges - m_CurCharges) )
					{
						...
						if ( jars ){ from.AddToBackpack( new Jar( ( m_MaxCharges - m_CurCharges ) ) ); }
					}
					else
					{
						m_CurCharges += amount;
