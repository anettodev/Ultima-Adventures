...
namespace Server.Spells.First
{
	...
	public class ClumsySpell : MagerySpell
	{
		...
		private const int EFFECT_EFFECT = 5002;
		private const int EFFECT_HUE_DEFAULT = 0;

		// Sound Constants
		private const int SOUND_ID = 0x1DF;
		...
		public void Target( Mobile target )
		{
			...
			else if ( CheckHSequence( target ) )
			{
				...
				BuffInfo.AddBuff( target, new BuffInfo( BuffIcon.Clumsy, 1075831, length, target, percentage.ToString() ) );
	...
	private bool HasSorcererImmunity( Mobile target )
	{
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( ClumsySpell owner ) : base( Core.ML ? TARGET_RANGE_ML : TARGET_RANGE_LEGACY, false, TargetFlags.Harmful )
			{
				m_Owner = owner;
			}
