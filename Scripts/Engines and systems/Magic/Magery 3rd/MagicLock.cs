...
namespace Server.Spells.Third
{
	...
	public class MagicLockSpell : MagerySpell
	{
		...
		// DEPRECATED - Old fame/karma adjustment method
		private void IncreaseOrDecreaseFameKarma(BaseCreature bc)
					{
                        // You cannot cast this on a locked down item.
                        Caster.PlaySound(Caster.Female ? 812 : 1086);
                        Caster.Say("*oops*");
                        Caster.SendMessage(95, "Voc� n�o pode lan�ar um feiti�o em um ba� seguro!");
					...
					else if ( box.Locked ||  box is ParagonChest || box is TreasureMapChest || box is PirateChest)
					{
                        ...
                        Caster.PlaySound(Caster.Female ? 812 : 1086);
                        Caster.Say("*oops*");
                        Caster.SendMessage(95, "Voc� n�o pode lan�ar um feiti�o em um ba� j� trancado.");
					...
					else if ( CheckSequence() )
					{
                        ...
                        Caster.SendMessage(2253, "O ba� foi trancado com magia!");
                        Caster.PlaySound(Caster.Female ? 779 : 1050);
                        Caster.Say("*ah ha!*");
				...
				else if ( targ is BaseDoor )
				{
					...
					if (Server.Items.DoorType.IsDungeonDoor(door))
					{
						if (door.Locked == true)
						{
							Caster.PlaySound(Caster.Female ? 812 : 1086);
							Caster.Say("*oops*");
							Caster.SendMessage(95, "Essa porta j� est� trancada!");
						}
						else
						{
							SpellHelper.Turn(Caster, door);
					...
					else 
					{
                        Caster.PlaySound(Caster.Female ? 812 : 1086);
                        Caster.Say("*oops*");
                        Caster.SendMessage(95, "Este feiti�o n�o tem efeito sobre essa porta!");
                    }
				}
				else
				{
                    Caster.PlaySound(Caster.Female ? 812 : 1086);
                    Caster.Say("*oops*");
                    Caster.SendMessage(95, "Este feiti�o n�o tem efeito sobre isso!");
                }
			...
			else if ( o is PlayerMobile )
			{
                ...
                FailToCaptureTheSoul(95, "Esta alma � forte demais para ficar presa no frasco!");
            }
			else if ( o is BaseCreature )
			{
                ...
                if (Caster.Backpack.FindItemByType(typeof(ElectrumFlask)) == null)
                {
                    Caster.SendMessage(55, "Voc� precisa de um frasco de electrum vazio!");
                }
                else if ( !bc.Alive )
				{
                    FailToCaptureTheSoul(95, "Voc� n�o pode capturar algo que est� morto!");
                }
				else if ( o is LockedCreature )
				{
                    FailToCaptureTheSoul(95, "Este ser j� foi preso uma vez e n�o se deixar� ser subjugado novamente!");
                }
				else if ( bc.Controlled )
				{
                    FailToCaptureTheSoul(95, "Este ser j� est� est� sob o controle de outro!");
                }
				...
				{
                    FailToCaptureTheSoul(95, "Este ser � protegido por uma aura misteriosa.");
                }
/*				else if ( (double)bc.Fame / 30000 > (double)Caster.Fame / 15000 )
				{
					Caster.SendMessage(95, "Voc� falhou em capturar alma deste ser por ele ter uma reputa��o elevada.");
				}*/
				else if ( bc.EmoteHue == 505 || bc.ControlSlots >= 100 ) // SUMMON QUEST AND QUEST MONSTERS
				{
                    FailToCaptureTheSoul(95, "Voc� n�o � capaz o suficiente para captura-lo!");
				}
				else if (bc is EpicCharacter || bc is TimeLord || bc is TownGuards)
				{
                    bc.Say("Voc� acha que pode capturar minha alma?");
                    bc.Say("Deixe-me mostrar o que posso fazer com voc�, seu inseto!");
                    Caster.PlaySound(Caster.Female ? 799 : 1071);
                    Caster.Say("*huh?*");
                    FailToCaptureTheSoul(55, "Voc� sente uma for�a poderosa bater em voc�!");
				...
				else
				{
                    ...
                    if (playerMagery >= 100 && playerEval >= 100 && playerInt >= 100)
					{
						...
						if (Caster.Mana > requiredMana) 
						{
                            ...
                            if (successPercentage >= random)
                            {
                                Misc.Titles.AwardFame(Caster, (30), true); // ganha fama pela a�ao
                                ...
                                Caster.SendMessage(55, "Voc� capturou a alma de " + bc.Name + " em um frasco de electrum!");
                            }
                            else
                            {
								FailToCaptureTheSoul(95, "Voc� falha na tentativa de capturar a alma deste ser!");
                            }
                        }
						else 
						{
							FailToCaptureTheSoul(95, "Voc� n�o possui mana o suficiente!");
                        }
                    }
					else {
						FailToCaptureTheSoul(95, "Voc� n�o possui habilidades o suficiente para capturar essa alma.");
                    }
				}
			}
