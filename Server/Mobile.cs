	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
	{
		...
		public virtual void ComputeResistances()
		{
			...
			for( int i = 0; i < m_Resistances.Length; ++i )
			{
				...
				if( m_Player )
					m_Resistances[i] = DiminishingReturns( m_Resistances[i], (max + 75) );
		...
		public virtual int DiminishingReturns( int Value, int Max )
		{
			double final = 0; // the final value
			double step = (double)Max / 10;
			double compute = (double)Value;

			if ((double)Value < step)
			{
				final = (double)Value;	
			}	
			else 
			{	
				while ( compute > 0 )
				{
					...
					{
						final += compute * (1 - ( final / (double)Max));
						compute = 0;
					}
				}
			}
		...
		public virtual bool CanUseStuckMenu()
		{
			...
			else
			{
				...
				{
					if ((DateTime.UtcNow - this.m_StuckMenuUses[i]) > TimeSpan.FromDays(1.0))
					{
						return true;
					}
				}
		...
		{
			return AccessLevel == AccessLevel.Administrator;
		}
